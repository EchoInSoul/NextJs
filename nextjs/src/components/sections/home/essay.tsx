...
const essayList = [
  "快写一首情歌，雅俗共赏～",
  "深圳欢乐谷是真好玩!!!",
  "React Server Components 真是太棒了！",
  "Next.js 15 带来了很多新特性",
  "今天天气真不错，出去走走吧"
];

export default function EssayBar() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentIndex((prev) => (prev + 1) % essayList.length);
    }, 3000);
  ...
  return (
    <div className="essay-bar-wrapper flex items-center w-full max-w-[calc(1400px-3rem)] h-[50px] px-4 py-2 mx-auto font-bold text-[var(--foreground)] bg-[var(--card)] border border-[var(--border)] rounded-xl shadow-[0_8px_16px_-4px_rgba(0,0,0,0.05)] transition-all duration-300 hover:border-[var(--primary)] mb-4">
      {/* Logo */}
      <Link 
        href="/essay" 
        className="essay-logo flex items-center mr-4 text-2xl leading-none text-[var(--foreground)] no-underline transition-all duration-300 hover:text-[var(--primary)]"
        title="即刻短文"
      >
        <svg 
      ...
      <Link 
        href="/essay" 
        className="essay-more-btn flex shrink-0 items-center justify-center ml-2 text-[var(--foreground)] cursor-pointer transition-all duration-300 hover:text-[var(--primary)] hover:translate-x-1"
        title="查看全文"
      >
        <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="20" height="20" viewBox="0 0 512 512">
          <path fill="currentColor" d="M0 256a256 256 0 1 0 512 0a256 256 0 1 0-512 0m297 129c-9.4 9.4-24.6 9.4-33.9 0s-9.4-24.6 0-33.9l71-71L120 280c-13.3 0-24-10.7-24-24s10.7-24 24-24h214.1l-71-71c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0L409 239c9.4 9.4 9.4 24.6 0 33.9z"></path>
        </svg>
      </Link>
    </div>
  );
